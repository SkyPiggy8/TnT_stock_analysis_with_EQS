...
- 更新最新交易日的热点数据，并导出不依赖后端的单文件交互 HTML。
...
页面会根据 Tushare 交易日历开放可选日期，即使某个已开市日尚无本地缓存，也可选中后点击“下载并生成该日热点榜”。当天 19:00 后首次打开热点工作区时，浏览器会自动刷新一次当日数据；也可随时点击“更新最新交易日数据”手动刷新。重新获取只覆盖目标交易日，不会重新下载全部 120 个交易日。

热点榜加载完成后，点击“导出交互 HTML”可下载一个单独的 `.html` 文件。文件内已嵌入当日数据、样式和交互脚本，可直接发给其他人；对方无需安装 Python，可离线打开并使用板块筛选、关键字搜索、榜单切换和排序。
